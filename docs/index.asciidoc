...
For example, if you configure this plugin with `metrics => "mymetrics"`, and "mymetrics" is a nested hash of '{a => 1, b => { c => 2 }}' this plugin generates two metrics: a => 1, and b.c => 2 .

If you've specified a 'metrics_format' it will be respected, but you still may want control over the separator within these nested key names.
